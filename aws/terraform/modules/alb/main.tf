...
# ── ALB log delivery bucket policy (required for ALB to write access logs) ────
...
resource "aws_s3_bucket_policy" "alb_logs" {
  ...
  depends_on = [aws_s3_bucket_server_side_encryption_configuration.alb_logs]
}
...
resource "aws_lb_target_group" "gateway" {
  ...
  tags = { Name = "${var.name_prefix}-gateway-tg" }
}
