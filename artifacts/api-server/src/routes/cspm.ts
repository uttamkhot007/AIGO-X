...
import { cloudResourcesTable, cloudFindingsTable, cspmDriftEventsTable, cloudIntegrationsTable } from "@workspace/db";
...
router.get("/cspm/stats", requireAuth, async (req, res) => {
  try {
    ...
    const integrations = await db.select().from(cloudIntegrationsTable).where(eq(cloudIntegrationsTable.tenantId, tenantId));
    const connected = integrations.length > 0;
    if (!connected) {
      res.json({ totalResources: 0, compliantResources: 0, byProvider: { AWS: 0, Azure: 0, GCP: 0 }, findings: { total: 0, critical: 0, high: 0, medium: 0, open: 0 }, driftItems: 0, complianceScore: 0, connected: false, integrations: [] });
      return;
    }
    const [resources, findings, drift] = await Promise.all([
      ...
      db.select().from(cspmDriftEventsTable).where(eq(cspmDriftEventsTable.tenantId, tenantId)),
    ]);
    res.json({
      ...
      driftItems:      drift.filter(d => !d.acknowledged).length,
...
router.get("/cspm/resources", requireAuth, async (req, res) => {
  try {
    const tenantId = tid(req);
    const integrations = await db.select().from(cloudIntegrationsTable).where(eq(cloudIntegrationsTable.tenantId, tenantId));
    if (integrations.length === 0) { res.json([]); return; }
    const { provider } = req.query as Record<string, string | undefined>;
    let rows = await db.select().from(cloudResourcesTable).where(eq(cloudResourcesTable.tenantId, tenantId));
...
router.get("/cspm/findings", requireAuth, async (req, res) => {
  try {
    ...
    let rows = await db.select().from(cloudFindingsTable).where(eq(cloudFindingsTable.tenantId, tenantId));
...
router.get("/cspm/drift", requireAuth, async (req, res) => {
  try {
    const { resourceId } = req.query as Record<string, string | undefined>;
    let rows = await db.select().from(cspmDriftEventsTable).where(eq(cspmDriftEventsTable.tenantId, tid(req)));
    if (resourceId) rows = rows.filter(d => d.resourceId === resourceId);
    res.json(rows);
  } catch { res.status(500).json({ error: "Internal server error" }); }
});

router.post("/cspm/drift/:id/acknowledge", requireAuth, async (req, res) => {
  try {
    const driftId = String(req.params["id"] ?? "");
    const [row] = await db.update(cspmDriftEventsTable)
      .set({ acknowledged: true, updatedAt: new Date() })
      .where(and(eq(cspmDriftEventsTable.tenantId, tid(req)), eq(cspmDriftEventsTable.driftId, driftId)))
      .returning();
    if (!row) { res.status(404).json({ error: "Drift record not found" }); return; }
    res.json(row);
  } catch { res.status(500).json({ error: "Internal server error" }); }
});
