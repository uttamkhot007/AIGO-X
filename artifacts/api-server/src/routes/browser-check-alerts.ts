...
router.post("/browser-check-alerts/test", requireAuth, async (req, res) => {
  try {
    ...
    await sendBrowserCheckAlerts(tid, {
      ...
      controlRef:    "TEST-001",
      screenshotUrl: null,
      errorMessage:  "This is a test alert from AIGO.",
    });

    return res.json({ success: true, message: "Test alert sent. Check your Slack/email." });
  } catch (err) {
    return res.status(500).json({ error: err instanceof Error ? err.message : "Failed to send test alert" });
  }
});
