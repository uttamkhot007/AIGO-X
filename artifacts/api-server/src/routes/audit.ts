  ...
  try {
    ...
    for (let i = pages.start; i < pages.start + pages.count; i++) {
      ...
      doc.fillColor("#9CA3AF").fontSize(7).font("Helvetica")
         .text(`AIGO-X GRC Platform  ·  Confidential  ·  Page ${i + 1}`, 50, doc.page.height - 30, { align: "center", width: doc.page.width - 100 });
    }
...
// ── Audit Programs Seed Data ──────────────────────────────────────────────────
const SEED_AUDIT_PROGRAMS: any[] = [
  { id:"AUD-001", name:"ISO 27001 Surveillance Audit 2024",   framework:"ISO 27001",    type:"Surveillance",    auditor:"BSI Group",       scheduled:"2024-03-15", status:"in-progress", scope:"Full ISMS — all Annex A controls",             findings:4 },
  { id:"AUD-002", name:"SOC 2 Type II Annual Review",         framework:"SOC 2",        type:"External",        auditor:"Deloitte & Touche",scheduled:"2024-06-01", status:"in-progress", scope:"Trust Services — Security + Availability",    findings:1 },
  { id:"AUD-003", name:"PCI DSS v4.0 SAQ-D Completion",       framework:"PCI DSS v4.0", type:"Self-Assessment", auditor:"Internal — QSA",  scheduled:"2024-04-10", status:"completed",   scope:"Cardholder data environment — DC1",           findings:0 },
  { id:"AUD-004", name:"GDPR Data Flow Audit",                framework:"GDPR / DPDPA", type:"Internal",        auditor:"Privacy Team",    scheduled:"2024-07-01", status:"planned",     scope:"All EU personal data processing activities",  findings:0 },
  { id:"AUD-005", name:"NIST CSF 2.0 Maturity Assessment",    framework:"NIST CSF 2.0", type:"Internal",        auditor:"Security Team",   scheduled:"2024-08-01", status:"planned",     scope:"Govern / Identify / Protect / Detect / Respond",findings:0},
  { id:"AUD-006", name:"RBI Cybersecurity Framework Review",  framework:"RBI CSF",      type:"Regulator",       auditor:"RBI CSITE Team",  scheduled:"2024-09-15", status:"planned",     scope:"IT governance + CSITE thematic review",       findings:0 },
];;
router.get("/audit/programs", requireAuth, (_req, res) => { res.json(SEED_AUDIT_PROGRAMS); });
