...
export async function sendViaSlack(webhookUrl: string, text: string, period: string): Promise<void> {
  ...
  const payload = {
    text: `*AIGO AI Security Briefing — ${period}*`,
    blocks: [
      {
        type: "section",
        text: {
          type: "mrkdwn",
          text: `*AIGO AI vCISO Security Briefing*\n_Period: ${period}_`,
        },
      },
      { type: "divider" },
      {
        ...
        ? [
            {
              ...
              text: {
                type: "mrkdwn",
                text: `_[Briefing truncated — log into AIGO to view the full report]_`,
              },
            },
          ]
        : []),
...
<h1 style="color:#1E3A5F;border-bottom:2px solid #1E3A5F;padding-bottom:12px;">
  AIGO AI vCISO Security Briefing
</h1>
...
<p style="font-size:12px;color:#94a3b8;">Delivered by AIGO-X GRC Platform · AI-powered security intelligence</p>
</body></html>`;

  const payload = {
    personalizations: [{ to: [{ email: toEmail }] }],
    from: { email: process.env["SENDGRID_FROM_EMAIL"] ?? "noreply@aigo-x.com", name: "AIGO AI vCISO" },
