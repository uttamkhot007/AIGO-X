...
function buildSlackBlocks(summary: AlertRunSummary): object[] {
  ...
  const blocks: object[] = [
    {
      type: "header",
      text: { type: "plain_text", text: "AIGO Evidence Alert", emoji: true },
    },
    {
      type: "section",
  ...
  blocks.push({
    ...
    text: {
      type: "mrkdwn",
      text: `<${deepLink}|→ Open Evidence Engine in AIGO>`,
    },
  });
...
function buildEmailHtml(summary: AlertRunSummary): { subject: string; html: string; text: string } {
  ...
  const subject = failed > 0
    ? `[AIGO Alert] ${failed} control${failed !== 1 ? "s" : ""} failed evidence collection`
    : `[AIGO Alert] ${stale} control${stale !== 1 ? "s" : ""} have stale evidence`;
...
<div style="background:#1E3A5F;padding:24px 32px;border-radius:12px 12px 0 0;">
  <h1 style="color:#fff;margin:0;font-size:22px;">⚠️ AIGO Evidence Alert</h1>
...
<div style="background:#F8FAFC;padding:16px 32px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px;text-align:center;">
  <p style="font-size:12px;color:#94A3B8;margin:0;">Delivered by AIGO-X GRC Platform · Evidence Alert Service</p>
</div>
</body></html>`;

  const text = `AIGO Evidence Alert\n\nFailed: ${failed} | Stale: ${stale} | Run: ${runId}\n\nAffected Controls:\n${
...
async function deliverAlert(
  ...
  try {
    if (channel === "slack") {
      ...
      const fallbackText = `AIGO Alert: ${summary.failed} control(s) failed evidence collection (run ${summary.runId})`;
      ...
      const payload = {
        ...
        from: { email: process.env["SENDGRID_FROM_EMAIL"] ?? "noreply@aigo-x.com", name: "AIGO Evidence Alerts" },
        subject,
        content: [
