...
function buildSlackBlocks(p: BrowserCheckAlertPayload): object[] {
  ...
  const blocks: object[] = [
    {
      type: "header",
      text: { type: "plain_text", text: "DuFense Browser Check Alert", emoji: true },
    },
    {
      type: "section",
      text: {
  ...
  blocks.push({
    ...
    text: {
      type: "mrkdwn",
      text: `<${deepLink}|→ Open Control in DuFense>`,
    },
  });
...
function buildEmailHtml(p: BrowserCheckAlertPayload): { subject: string; html: string; text: string } {
  ...
  const subject = `[DuFense Alert] Browser check "${p.checkName}" transitioned to ${verdictLabel}`;
...
<div style="background:#1E3A5F;padding:24px 32px;border-radius:12px 12px 0 0;">
  <h1 style="color:#fff;margin:0;font-size:20px;">❌ DuFense Browser Check Alert</h1>
...
<div style="background:#fff;padding:24px 32px;border:1px solid #E2E8F0;border-top:none;">
  ...
  <div style="margin-top:28px;text-align:center;">
    <a href="${deepLink}" style="background:#1E3A5F;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;">Open Control in DuFense →</a>
  </div>
</div>
<div style="background:#F8FAFC;padding:16px 32px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px;text-align:center;">
  <p style="font-size:12px;color:#94A3B8;margin:0;">Delivered by DuFense GRC Platform · Browser Check Alert Service</p>
</div>
</body></html>`;

  const text = `DuFense Browser Check Alert\n\nCheck: ${p.checkName}\nURL: ${p.url}\nControl: ${p.controlRef}\nVerdict: ${verdictLabel}\nRun ID: ${p.runId}${p.errorMessage ? `\nError: ${p.errorMessage}` : ""}${p.screenshotUrl ? `\nScreenshot: ${screenshotViewUrl}` : ""}\n\nOpen in DuFense: ${deepLink}`;
...
async function deliverAlert(
  ...
  try {
    if (channel === "slack") {
      ...
      const fallbackText = `DuFense Alert: Browser check "${payload.checkName}" transitioned to ${payload.verdict.toUpperCase()} (run ${payload.runId})`;
      ...
      const emailPayload = {
        ...
        from: {
          email: process.env["SENDGRID_FROM_EMAIL"] ?? "noreply@dufense.io",
          name: "DuFense Browser Check Alerts",
        },
        subject,
        content: [
