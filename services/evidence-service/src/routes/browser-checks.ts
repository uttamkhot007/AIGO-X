...
router.get(/^\/objects\/(.+)$/, requireAuth, async (req, res) => {
  try {
    ...
    if (!artifact) {
      return res.status(403).json({ error: "Forbidden: object does not belong to your tenant" });
    }
    ...
    if (code === "403") return res.status(403).json({ error: "Forbidden" });
...
router.get("/controls/:controlId/browser-checks", requireAuth, async (req, res) => {
  try {
    const { tenantId } = (req as AuthReq).user;
    const tid = Number(tenantId);
    const ctrl = await resolveControl(tid, req.params.controlId!);
  ...
  try {
    ...
    await db.delete(browserChecksTable)
      .where(and(
        ...
        eq(browserChecksTable.checkId, req.params.checkId!),
      ));
    res.json({ ok: true });
  } catch { res.status(500).json({ error: "Internal server error" }); }
});
...
router.get("/controls/:controlId/browser-checks/export/pdf", requireAuth, async (req: Request, res: Response) => {
  try {
        ...
        for (const run of checkRuns) {
          ...
          if (screenshotUrl) {
            ...
            if (imgBuf) {
              if (doc.y > 650) doc.addPage();
              try {
                doc.image(imgBuf, 74, doc.y, { width: Math.min(PAGE_W - 24, 420), align: "left" });
                doc.moveDown(0.5);
              } catch {
    ...
    doc.fontSize(8).font("Helvetica").fillColor(GRAY)
      .text(`Generated by DuFense GRC Platform — ${new Date().toISOString()} — Confidential`, { align: "center" });
...
router.get("/browser-check-runs/:runId", requireAuth, async (req, res) => {
  try {
    ...
    const [run] = await db.select().from(browserCheckRunsTable)
      .where(and(
        eq(browserCheckRunsTable.tenantId, tid),
        eq(browserCheckRunsTable.runId, req.params.runId!),
      ));
