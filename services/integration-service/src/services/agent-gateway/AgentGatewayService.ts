...
import { db, agentRefreshTokensTable, agentRecordsTable } from "@workspace/db";
...
interface EnrollTokenRecord {
  tenantId: number;
}

const ALL_MODULE_FEEDS = ["complyops", "assetops", "dataops", "secops", "serviceops"];

const DEFAULT_POLICY: AgentPolicy = {
  scanSchedule: "0 */4 * * *",
  reportingIntervalSecs: 60,
  dataTypes: ["inventory", "events", "vulnerabilities"],
  moduleFeeds: ALL_MODULE_FEEDS,
  logLevel: "info",
  maxCpuPct: 15,
  maxMemMb: 256,
};

const DEFAULT_STALE_SECS = 300;

class AgentGatewayService {
  private agents = new Map<string, AgentRecord>();
  private enrollTokens = new Map<string, EnrollTokenRecord>();
  ...
  constructor() {
    ...
    this._seedEnrollTokens();
    this._seed();
    this._ready = this._loadFromDb();
  }

  private _seedEnrollTokens() {
    const envToken = process.env["AIGO_ENROLL_TOKEN"];
    if (!envToken) {
      console.warn("[AgentGatewayService] AIGO_ENROLL_TOKEN is not set — agent enrollment is disabled until this is configured");
      return;
    }
    this.enrollTokens.set(envToken, { tenantId: 1 });
  }

  validateEnrollToken(token: string): EnrollTokenRecord | null {
    return this.enrollTokens.get(token) ?? null;
  }
    ...
    const seeds: Array<Omit<AgentRecord, "id" | "registeredAt" | "hmacSecret">> = [
      {
        ...
        policy: { ...DEFAULT_POLICY, moduleFeeds: ALL_MODULE_FEEDS },
        pendingPush: null,
      ...
      },
      {
        ...
        policy: { ...DEFAULT_POLICY, dataTypes: ["ad-inventory", "gpo", "events"], moduleFeeds: ["complyops", "secops"] },
        pendingPush: null,
      ...
      },
      {
        ...
        policy: { ...DEFAULT_POLICY, moduleFeeds: ["assetops", "dataops", "secops"] },
        pendingPush: null,
      ...
      },
      {
        ...
        policy: { ...DEFAULT_POLICY, moduleFeeds: ["secops", "serviceops"] },
        pendingPush: null,
      ...
      },
      {
        ...
        policy: { ...DEFAULT_POLICY, dataTypes: ["mdm-compliance", "app-inventory"], moduleFeeds: ["complyops", "assetops", "serviceops"] },
        pendingPush: null,
  ...
  async register(tenantId: string, payload: RegisterPayload): Promise<AgentRecord> {
  ...
  async consumeRefreshToken(token: string): Promise<RefreshTokenRecord | null> {
    const rows = await db
      ...
      .where(
        eq(agentRefreshTokensTable.token, token),
      )
      .returning();
    if (rows.length === 0) return null;
    const row = rows[0]!;
    if (row.expiresAt < new Date()) return null;
    return { agentId: row.agentId, tenantId: row.tenantId, expiresAt: row.expiresAt.getTime() };
  }
