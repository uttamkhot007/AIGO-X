...
import { db, verifyToken } from "@workspace/service-kit";
import {
...
const MCP_TOOLS = [
  ...
  {
    name: "ask_vciso",
    description: "Ask the DuFense AI vCISO a question about the tenant's GRC posture, security strategy, compliance, or risk management. The vCISO has live access to the tenant's risk register, control statuses, and findings.",
    inputSchema: {
      type: "object",
      properties: {
...
router.get("/mcp/manifest", (_req, res) => {
  res.json({
    name: "dufense-grc",
    version: "2.0.0",
    displayName: "DuFense GRC Platform",
      ...
      case "initialize":
        res.json({
          ...
          result: {
            ...
            serverInfo: { name: "dufense-grc", version: "2.0.0" },
          },
        });
        break;
