...
router.post("/ai/enrich", requireAuth, async (req, res) => {
  ...
  const prompt = `You are DuFense AI vCISO. Analyse this GRC object and provide a JSON enrichment response.
...
function renderMarkdownToPdf(doc: InstanceType<typeof PDFDocument>, markdown: string, title?: string) {
  ...
  if (title) {
    ...
    doc.fontSize(9).font("Helvetica").fillColor("#6B7280").text(`Generated by DuFense AI vCISO · ${new Date().toLocaleDateString()}`, left, doc.y + 4, { width: pageWidth });
    doc.moveDown(1);
...
function renderMarkdownToDocx(markdown: string, title?: string): Paragraph[] {
  ...
  if (title) {
    ...
    paragraphs.push(new Paragraph({ children: [new TextRun({ text: `Generated by DuFense AI vCISO · ${new Date().toLocaleDateString()}`, color: "6B7280", size: 18 })] }));
